...
export default function InventoryPage() {
  ...
  const downloadInventoryTemplate = () => {
    ...
    const example: Record<string, unknown> = {
      PROVEEDOR: "VALERIA ZUBIETA",
      FABRICANTE: "DEPO",
      PRODUCTO: "BISEL TOYOTA COROLLA 84-87 VAGONETA LH",
      MARCA: "TOYOTA",
      MODELO: "COROLLA",
      AÑO: "1984-1987",
      DETALLES: "",
      "COD OEM": "100000-100000",
      "COD FABRICA": "11-11920005B3",
      "COSTO $": 9.9,
      "COSTO BS": 148.95,
      "COSTO TIENDAS": 158.88,
      "PRECIO 1": 250,
      "PRECIO 2": 230,
      "PRECIO MAYOR": 200,
      IMAGEN: "",
    };
    locations.forEach((l) => { example[l.name] = ""; });
    const ws = XLSX.utils.json_to_sheet([example], { header: headers });
    ws["!cols"] = headers.map((h) => ({ wch: Math.max(h.length, String(example[h] ?? "").length) + 2 }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Plantilla");
    XLSX.writeFile(wb, "Plantilla_Actualizacion_Inventario.xlsx");
...
const handleImportExcel = async () => {
  ...
  const downloadImportLog = () => {
    ...
    const rows: string[] = [];
    rows.push("=== LOG DE IMPORTACIÓN ===");
    rows.push(`Fecha: ${new Date().toLocaleString()}`);
    rows.push(`Tipo: ${importType === "depo" ? "Importar 1 (catálogo)" : "Importar 2 (actualizar inventario)"}`);
    rows.push(`Archivo: ${importFile?.name ?? "—"}`);
    rows.push("");
    rows.push("=== RESUMEN ===");
    ...
    const list = (title: string, subtitle: string, items: any[]) => {
      ...
      items.forEach((it: any) => {
        const rowLabel = it.row ?? "";
        if (subtitle === "creado" || subtitle === "actualizado") {
          rows.push(`- [${subtitle.toUpperCase()}] ${rowLabel ? `${rowLabel} - ` : ""}${it.itemCode ?? ""}${it.name ? ` · ${it.name}` : ""}`);
        } else {
          rows.push(`- ${it}`);
        }
      });
    ...
    list("CREADOS", "creado", det.imported ?? []);
    list("ACTUALIZADOS", "actualizado", det.updated ?? []);
    list("ERRORES", "error", det.errors ?? []);
    list("ADVERTENCIAS", "warning", det.warnings ?? []);
    const blob = new Blob([rows.join("\n")], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `log_importacion_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.txt`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    toast.success("Log de importación descargado");
  };
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div ref={importPanelRef} role="dialog" aria-modal="true" aria-label="Importar productos desde Excel" className="bg-dark-800 border border-dark-700/50 rounded-2xl w-full max-w-lg">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setImportType("depo")}
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-dark-700/50">
              {importResult && (
                <button onClick={downloadImportLog} className="px-4 py-2.5 text-sm bg-dark-700 hover:bg-dark-600 text-gray-200 rounded-xl transition-all flex items-center gap-2">
                  <Download size={16} /> Descargar log
                </button>
              )}
